...
def config_info(config_id):
    config_item = config.co_config.find_one({"_id": config_id})
    config_item["cnt"] = config.co_detail.count(query={"config_id": config_id})
    config_item["valid_cnt"] = config.co_detail.count(query={"config_id": config_id, "code": config.HTTP_OK_CODE})
    if 'type' in request.args and request.args['type'] == 'json':
...
def config_invalid():
    ...
    try:
        # TODO:
        config_item = config.co_config.find_one({"_id": config_id})
        config.co_config.update_by_id(config_id, {"state": 0})
        config.co_scanner.delete_by_id(config_id)
        config.co_alerter.delete_by_id(config_id)
        res["msg"] = "操作成功"
    except Exception as e:
        res["code"] = 500
...
def config_add_get():
    groups = config.co_config_group.get_all(all=False)
    return render_template('add.html', groups=groups)


@mod.route("/update/<config_id>", methods=['GET'])
def config_update_get(config_id):
    config_item = config.co_config.find_one({"_id": config_id})
    return render_template('update.html', res=config_item)


# @mod.route("/set/receivers", methods=['GET'])
# def config_set_receivers():
#     res = APIMixin().api_res
#     configs = config.co_config.get_all()
...
#             }]
#         logger.debug(data)
#         config.co_config.update_by_id(config["_id"], data)
#     return json.dumps(res)
...
def config_add():
    ...
    try:
        ...
        res["code"] = 400
        return json.dumps(res)        
    doc = config.co_config.new_doc_object() 
    doc.update({
        "app_name": app_name,
        "group": group,
        "desc": desc,
    ...
    try:
        logger.debug(doc)
        config.co_config.insert_one(doc)
        res["msg"] = "新增api成功"
...
def config_update(config_id):
    res = APIMixin().api_res
    if not config.co_config.exists(config_id):
        msg = "Config id <%s> not existed" % config_id
        res["msg"] = msg
        res["code"] = 400
        return json.dumps(res)
    config_item = config.co_config.get(config_id)
    try:
        ...
        res["code"] = 400
        return json.dumps(res)
    if not config.co_config.update(config_id, request_data):
        msg = "Update config <%s> failed" % config_id
        res["msg"] = msg
        res["code"] = 500
        return json.dumps(res)

    new_config_item = config.co_config.get(config_id)
